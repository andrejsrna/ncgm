...
export default function StreamingPlatformsSection({ track }: StreamingPlatformsSectionProps) {
  ...
  const links = [
    ...
    { name: "Apple Music", url: track.AppleMusic, icon: <FaApple className="h-5 w-5 text-slate-700" /> },
  ...
  return (
    ...
    >
      <h2 id="streaming-options" className="text-sm font-semibold uppercase tracking-[0.3em] text-slate-500">
        ...
        {links.map((link) => (
          <li key={link.name}>
            <a
              ...
              className="flex items-center gap-3 rounded-xl border border-border bg-slate-50 px-4 py-3 transition hover:border-primary hover:bg-white"
            >
              {link.icon}
              <span>{link.name}</span>
            </a>
